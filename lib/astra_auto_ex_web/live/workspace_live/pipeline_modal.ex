defmodule AstraAutoExWeb.WorkspaceLive.PipelineModal do
  ...
  @default_status_messages ["准备中...", "思考中...", "写作中...", "润色中...", "即将完成..."]
  ...
  def mount(socket) do
    {:ok,
     ...
     |> assign_new(:status_messages, fn -> @default_status_messages end)
  ...
  def render(assigns) do
    ...
    <div>
      <div :if={@active} class="fixed inset-0 z-50 flex items-center justify-center">
        <%!-- Backdrop --%> <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" />
        <%!-- Modal --%>
        <div class="relative glass-card p-8 max-w-md w-full mx-4 text-center">
          ...
          <div class="inline-flex items-center justify-center w-20 h-20 rounded-full mb-6 relative">
            ...
            <svg
              class="w-8 h-8 text-[var(--glass-accent-from)]"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
  ...
  # Used via @status_messages module attribute in render
